...
function get_cell_index(target_board, col, row) {
    for (let i = 0; i < target_board.cells.length; i++) {
        if (target_board.cells[i].col === col && target_board.cells[i].row === row) {
            return i;
        }
    }
    return -1;
...
function isMoveLegal(cells, pieces, from, fromCell, toCell) {
    // Check if the move is legal based on the game rules
    // Implementation not provided in the input code
    return true; // Placeholder
}

function get_available_piece_moves(target_board, from, player) {
    const moves = [];
    const directions = [
        { xmove: -1, ymove: -1 },
        { xmove: -1, ymove: 1 },
    ...
    directions.forEach(({ xmove, ymove }) => {
        let cell_index = get_cell_index(target_board, from.col + xmove, from.row + ymove);
        if (cell_index >= 0) {
            const to = target_board.cells[cell_index];
            if (isMoveLegal(target_board.cells, target_board.pieces, from, from, to)) {
                const move = { move_type: 'jump', piece: player, from: { col: from.col, row: from.row }, to: { col: to.col, row: to.row } };
                moves.push(move);
            }
        }
    });
...
function get_player_pieces(player, target_board) {
    return target_board.pieces.filter(piece => piece.state === player);
}

function get_available_moves(player, target_board) {
    const moves = [];
    const player_pieces = get_player_pieces(player, target_board);

    for (let i = 0; i < player_pieces.length; i++) {
        const from = player_pieces[i];
        const piece_moves = get_available_piece_moves(target_board, from, player);
        moves.push(...piece_moves);
    }

    // Prune non-jumps, if applicable
    const jump_moves = moves.filter(move => move.move_type === "jump");
...
function select_random_move(moves) {
    ...
    const selected_move = moves[index];
...
function alpha_beta_search(calc_board, limit) {
    ...
    const available_moves = get_available_moves(computer, calc_board);

    // Get max value for each available move
    const max = max_value(calc_board, available_moves, limit, alpha, beta);
    ...
    const max_move = best_moves.length > 1 ? select_random_move(best_moves) : best_moves[0];
...
function computerMove() {
    ...
    const simulated_board = copy_board(currentBoard);

    // Run algorithm to select next move
    const selected_move = alpha_beta_search(simulated_board, 8);
    console.log(`best move: ${selected_move.from.col}:${selected_move.from.row} to ${selected_move.to.col}:${selected_move.to.row}`);

    // Make computer's move
    const pieceIndex = getPieceIndex(currentBoard.pieces, selected_move.from.row, selected_move.from.col);
...
function jump_available(available_moves) {
    return available_moves.some(move => move.move_type === "jump");
}
...
function min_value(calc_board, human_moves, limit, alpha, beta) {
    ...
    if (human_moves.length > 0) {
        for (let i = 0; i < human_moves.length; i++) {
            const simulated_board = copy_board(calc_board);

            // Move human piece
            const human_move = human_moves[i];
            const pieceIndex = getPieceIndex(simulated_board.pieces, human_move.from.row, human_move.from.col);
            const piece = simulated_board.pieces[pieceIndex];
            simulated_board = movePiece(simulated_board, piece, human_move.from, human_move.to);

            // Get available moves for computer
            const computer_moves = get_available_moves(computer, simulated_board);

            const max_score = max_value(simulated_board, computer_moves, limit - 1, alpha, beta);
...
function max_value(calc_board, computer_moves, limit, alpha, beta) {
    ...
    if (computer_moves.length > 0) {
        for (let i = 0; i < computer_moves.length; i++) {
            ...
            const computer_move = computer_moves[i];
            const pieceIndex = getPieceIndex(simulated_board.pieces, computer_move.from.row, computer_move.from.col);
            const piece = simulated_board.pieces[pieceIndex];
            simulated_board = movePiece(simulated_board, piece, computer_move.from, computer_move.to);

            const human_moves = get_available_moves(player, simulated_board);

            const min_score = min_value(simulated_board, human_moves, limit - 1, alpha, beta);
...
function utility(target_board) {
    ...
    for (let i = 0; i < target_board.pieces.length; i++) {
        const piece = target_board.pieces[i];
        if (piece.row > -1) { // Only count pieces still on the board
            if (piece.state > 0) { // Human
                ...
                const human_pos = evaluate_position(piece.col, piece.row);
                ...
                const computer_pos = evaluate_position(piece.col, piece.row);
    ...
    const avg_human_pos = human_pos_sum / human_pieces;
    if (computer_pieces === 0) {
        computer_pieces = 0.00001;
    }
    const avg_computer_pos = computer_pos_sum / computer_pieces;
